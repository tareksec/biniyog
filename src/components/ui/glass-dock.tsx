...
if (typeof window !== "undefined") {
    try {
        const pluginName = "gsap/MorphSVGPlugin";
        // @ts-ignore
        import(/* @vite-ignore */ pluginName).then((plugin) => {
            gsap.registerPlugin(plugin.MorphSVGPlugin);
        }).catch(e => {
            // Ignored - plugin not installed
        });
    } catch (e) {
        // Ignored
    }
...
export const GlassDock = React.forwardRef<HTMLDivElement, GlassDockProps>(
    (
        ...
        const getTooltipPosition = (index: number) => index * 56 + 16; // slightly adjusted for new flex gaps if needed, but relative

        return (
            <div
                ref={ref}
                className={cn('w-full flex justify-center max-w-full', className)}
                {...props}
            >
                <div
                    className={cn(
                        "glass-dock relative flex gap-3 sm:gap-4 items-center px-4 py-2 sm:px-6 sm:py-3 rounded-full",
                ...
                >
                    {leftContent && (
                        <div className="flex-shrink-0 mr-2 sm:mr-4 flex items-center">{leftContent}</div>
                    )}
                                    ...
                                    {isHovered && (
                                        <motion.div
                                            ...
                                            animate={{ 
                                                ...
                                                y: -50, 
                                                x: '-50%',
                                                transition: { duration: 0.15, ease: [0.22, 1, 0.36, 1], delay: 0.05 } 
                                            }}
                                            exit={{ 
                                                opacity: 0, 
                                                scale: 0.95, 
                                        ...
                                        >
                                            <div
                                                className={cn(
                                                    ...
                                                    'shadow-md flex items-center justify-center font-bold',
